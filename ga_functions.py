...
def initialize_population(population_size, inital_num_features):
    """
    WORKING
    Initailizes a generation of `population_size` number of chromosomes with each chromosome being `inital_num_features` long.
...
def compute_fitness_score(init_pop, X_train, X_test, y_train, y_test):
    ...
    for index, chromosome in enumerate(init_pop):
        ...
        score = M.fitness_score_NN(
            chromosome, X_train, X_test, y_train, y_test)
    ...
    try:
        ...
        for i in range(len(current_pop)):
            for j in range(int(lswap/2)):
                current_pop[i][j], current_pop[i][lswap -
                                                  j] = current_pop[i][lswap - j], current_pop[i][j]
        return current_pop
    except:
        return current_pop
...
def generational_update(current_pop, improved_pop):
    """
    WORKING
    Parameters:
    current_pop: list of chromosomes which denotes the current population
...
def weak_parents_update(current_pop, fitness_scores, improved_pop):
    ...
    for _i in range(len(current_pop)//2):
        ...
        updated_pop.append(improved_pop[rand_int])
    return updated_pop
